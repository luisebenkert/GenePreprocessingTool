...
cat("----------TCGAbiolinks: START PROCESSING NEW _NORMAL_ QUERY: TCGA-SARC", "TCGA-GBM Transcriptome Profiling , data.type = Gene Expression Quantification, workflow = HTSeq - Counts, barcode = c()----------
")

try({query1 <- GDCquery(project= c("TCGA-SARC", "TCGA-GBM"), data.category = "Transcriptome Profiling", data.type = "Gene Expression Quantification", workflow = "HTSeq - Counts", barcode = c(""))
...
write.csv(transp_metadata1, file="./GDCdata/2018-01-19_12-06-54/TCGA-SARC_TCGA-GBM__GeneExpressionQuantification_HTSeq-Counts_metadata.csv")
...
write.csv(transp_sel_meta1, file="./GDCdata/2018-01-19_12-06-54/TCGA-SARC_TCGA-GBM__GeneExpressionQuantification_HTSeq-Counts_metadata_topfeatures.csv")
matrix1 <- t(assay(transdds1)) #creates the gene expression matrix with genes as rows
write.csv(matrix1, file="./GDCdata/2018-01-19_12-06-54/TCGA-SARC_TCGA-GBM__GeneExpressionQuantification_HTSeq-Counts.csv")
final_matrix1 <- t(assay(selected_features1)) #creates the gene expression matrix with genes as rows
write.csv(final_matrix1, file="./GDCdata/2018-01-19_12-06-54/TCGA-SARC_TCGA-GBM__GeneExpressionQuantification_HTSeq-Counts_topfeatures.csv")
})
